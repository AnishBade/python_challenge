...
def scrape_url(url, depth, max_depth, base_domain, downloaded_site_path, visited):
    ...
    local_file_path = url.replace(f"http://{base_domain}", "").replace(
        f"https://{base_domain}", ""
    )
    ...
    if local_file_path == "" or local_file_path.endswith("/"):
        local_file_path += (
            "index.html"  # Append 'index.html' for base domain or directory-like URLs
        )
    ...
    for link in soup.find_all("a"):
        ...
        if href and valid_url(href):
            ...
            if full_url not in visited:
                ...
                scrape_url(
                    full_url, depth + 1, max_depth, base_domain, downloaded_site_path, visited
                )
